...
class _SocialPageState extends State<SocialPage> with SingleTickerProviderStateMixin {
  ...
  void _toggleFollowUser(String username) {
    setState(() {
      ...
      if (discoverIndex != -1) {
        discoverPosts[discoverIndex]['isFollowing'] = !(discoverPosts[discoverIndex]['isFollowing'] ?? false);
      } else {
  ...
  void _viewUserProfile(String username) {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => UserProfilePage(username: username, posts: posts),
      ),
    );
  }
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        return MyPostItem(
          ...
          onFollowToggle: () => _toggleFollowUser(widget.sharedPosts[index]['username']),
          onViewProfile: () => _viewUserProfile(widget.sharedPosts[index]['username']),
        );
      },
    );
  }
        ...
        SliverToBoxAdapter(
          child: SizedBox(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                return MyGroupSuggestion(
                  group: groups[index],
                  onJoin: () => setState(() {
                    if (joinedGroups.contains(groups[index]['name'])) {
                      joinedGroups.remove(groups[index]['name']);
                    } else {
                      joinedGroups.add(groups[index]['name']);
                    }
                  }),
                  isJoined: joinedGroups.contains(groups[index]['name']),
                );
              },
            ),
          ),
