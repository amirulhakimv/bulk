...
class _SocialPageState extends State<SocialPage> with SingleTickerProviderStateMixin {
                      ...
                      Expanded(
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            return _buildCommentTile(comments[index], postIndex, index, setModalState);
                          },
                        ),
                      ),
                      _buildCommentInput(postIndex, setModalState, _commentController),
                    ],
                  ),
                );
              },
  ...
  Widget _buildCommentTile(Map<String, dynamic> comment, int postIndex, int commentIndex, StateSetter setModalState) {
    return Column(
          ...
          subtitle: Row(
            children: [
              ...
              GestureDetector(
                onTap: () {
                  setModalState(() {
                    _replyingTo = comment['username']; // Updated line
                  });
                },
          ...
          trailing: Row(
            ...
            children: [
              ...
              IconButton(
                ...
                onPressed: () {
                  setModalState(() {
                    toggleCommentLike(postIndex, commentIndex, 'currentUserId');
                  });
                },
              ),
            ],
          ...
          Padding(
            ...
            child: Column(
              children: comment['replies'].map<Widget>((reply) {
                return _buildReplyTile(reply, postIndex, commentIndex, comment['id'], setModalState);
              }).toList(),
            ),
          ),
      ],
  ...
  Widget _buildReplyTile(Map<String, dynamic> reply, int postIndex, int commentIndex, String parentId, StateSetter setModalState) {
    return ListTile(
      leading: CircleAvatar(
        backgroundColor: Colors.grey,
        radius: 12,
      ...
      subtitle: Row(
        children: [
          ...
          GestureDetector(
            onTap: () {
              setModalState(() {
                _replyingTo = reply['username']; // Updated line
              });
            },
      ...
      trailing: Row(
        ...
        children: [
          ...
          IconButton(
            ...
            onPressed: () {
              setModalState(() {
                toggleReplyLike(postIndex, commentIndex, parentId, reply['id'], 'currentUserId');
              });
            },
          ),
        ],
  ...
  Widget _buildCommentInput(int postIndex, StateSetter setModalState, TextEditingController controller) {
    return Container(
          ...
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: _replyingTo.isEmpty ? 'Add a comment...' : 'Replying to $_replyingTo...', // Updated line
          ...
          TextButton(
            onPressed: () {
              if (controller.text.isNotEmpty) {
                setModalState(() {
                  if (_replyingTo.isEmpty) { // Updated line
                    addComment(postIndex, controller.text);
                  } else {
                    // Find the comment we're replying to and add the reply
                    final commentIndex = posts[postIndex]['comments'].indexWhere((c) => c['username'] == _replyingTo); // Updated line
                    if (commentIndex != -1) {
                      addReply(postIndex, commentIndex, posts[postIndex]['comments'][commentIndex]['id'], controller.text);
                    }
                  }
                  controller.clear();
                  _replyingTo = ''; // Updated line
